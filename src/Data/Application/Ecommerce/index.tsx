...
import { HvacOrders, HvacSubscription } from "@/Types/AdminHvacType";
import {
  HvacOrderActions,
  HvacOrderId,
  MembershipActions,
...
const ProductListTableAction: React.FC<UserListActions> = ({ id }) => {
  ///admin/users/edit_user/${id}
  return (
    <div className="product-action">
      <Link href={`/admin/users/edit_user/${id}`}>
...
export const ProductListTableDataColumn = [
  {
    name: "Name",
    selector: (row: ProductListTableDataColumnType) =>
      `${row.first_name} ${row.last_name}`,
    sortable: true,
  },
  {
    name: "Email Address",
    selector: (row: ProductListTableDataColumnType) => `${row.user_email}`,
    sortable: true,
  },
  {
    name: "Verification",
    selector: (row: ProductListTableDataColumnType) =>
      `${row.user_status === true ? "Verified " : "Pending"}`,
    sortable: false,
  },
  {
    name: "Created Date",
  ...
  },
  {
    ...
    cell: (row: ProductListTableDataColumnType) => (
      <ProductListTableAction id={row.user_id} />
    ),
    width: "100px",
  },
];
...
const MembershipTableAction: React.FC<MembershipActions> = ({ id }) => {
  return (
    <div className="product-action">
      <Link href={`/admin/memberships/details/${id}`}>
        <SVG iconId="edit-content" />
      </Link>
    </div>
  );
...
const HvacSubsTableButton: React.FC<HvacSubButton> = ({ name }) => {
  ...
  return (
    <span
      className={`badge ${name == 'hold' ? 'badge-warning' : 'badge-success'}`}
...
export const MembershipListTableDataColumn = [
  {
    name: "Name",
    cell: (row: MembershipDetais) =>
      // `${row.users.first_name} ${row.users.last_name}`,
      <div className="product-names my-2"> <p> {row.users.first_name} {row.users.last_name} </p> </div>,
    sortable: false,
    width:"150px"
  },
  {
    name: "Plan Name",
    selector: (row: MembershipDetais) => `${row.membership_plans.plan_name}`,
    sortable: false,
    width:"270px"
  },
  {
    name: "Plan Frequency",
    selector: (row: MembershipDetais) =>
      `${row.membership_plans.plan_frequency}`,
    sortable: false,
    width:"100px"
  },
  {
  ...
  },
  {
    name: "Status",
    cell: (row: MembershipDetais) => (
      <HvacSubsTableButton name={row.status} />
    ),
    // selector: (row: MembershipDetais) => `${row.status}`,
    sortable: false,
      width:"80px"
  },
  {
  ...
  {
    name: "Action",
    cell: (row: MembershipDetais) => <MembershipTableAction id={row.user_id}/>,
    sortable: false,
  },
];

export const MembershipDeatilsTableDataColumn = [
  // {
  //   name: "Plan Name",
  //   selector: (row: Membership) => `${row.plan_id}`,
  //   sortable: true,
  // },
  {
    name: "Start Date",
  ...
  {
    name: "Status",
    selector: (row: Membership) => `${row.status}`,
    sortable: true,
    width: "100px",
  },
];

const HvacOrderEditTableAction: React.FC<HvacOrderActions> = ({ id }) => {
  ///admin/users/edit_user/${id}
  return (
    <div className="product-action">
      <Link href={`/admin/hvac/orders/order_details/${id}`}>Edit</Link>
      {/* <SVG iconId="trash1" /> */}
    </div>
  );
};

const HvacOrderIdTableAction: React.FC<HvacOrderId> = ({
  id,
  first_name,
  last_name,
}) => {
  return (
    <div className="">
      <Link href={`/admin/hvac/orders/order_details/${id}`}>{`#${id}`}</Link>
      <span style={{ marginLeft: "4px" }}>{` ${first_name} ${last_name}`}</span>
    </div>
  );
};

export const AdminHvacOrderTableDataColumn = [
  {
    name: "Order",
    cell: (row: HvacOrders) => (
      <HvacOrderIdTableAction
        id={row.order_id}
  ...
  },
  {
    name: "Product",
    width: "600px",
    selector: (row: HvacOrders) =>
      `${row.productsData?.map((el) =>
  ...
  {
    name: "Status",
    selector: (row: HvacOrders) => `${row.order_status}`,
  },
  {
    name: "Subscription ID",
    selector: (row: HvacOrders) => `${row.subscription_id}`,
    sortable: true,
    width: "100px",
  },
  {
    name: "Amount",
    cell: (row: HvacOrders) => {
      const getAmount = row.orderItems?.map((el) => el.price) || [];
      const totalAmount = getAmount.reduce((acc, price) => acc + price, 0);
      const formattedAmount = new Intl.NumberFormat("en-US", {
        style: "currency",
        currency: "USD",
        minimumFractionDigits: 2,
  ...
  },
  {
    name: "Date",
    selector: (row: HvacOrders) =>
      `${moment(row.order_date).format("MM/DD/YYY hh:mm")}`,
    sortable: true,
    width: "200px",
  },
  {
    name: "Action",
    cell: (row: HvacOrders) => <HvacOrderEditTableAction id={row.order_id} />,
    width: "150px",
  },
];
